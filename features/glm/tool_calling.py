#!/usr/bin/env python3
"""Feature test for StatefulLLMNode tool calling with GLM provider.
...
REQUIREMENTS:
- GLM_API_KEY in .env.local
- WezTerm must be running
- Claude CLI must be installed and authenticated

Run with: uv run python features/glm/tool_calling.py
"""
...
def get_model():
    return os.getenv("GLM_MODEL", "GLM-4.7")


def get_http_backend():
    return os.getenv("GLM_HTTP_BACKEND", "openai")
...
async def test_bash_tool():
    ...
    session = Session("glm-tool-test-1")
...
async def test_claude_tool():
    ...
    session = Session("glm-tool-test-2")
...
async def test_multiple_tools():
    ...
    session = Session("glm-tool-test-3")
...
async def test_tool_choice_none():
    ...
    session = Session("glm-tool-test-4")
    bash = BashNode(id="bash", session=session, timeout=30.0)
    tools, executor = tools_from_nodes([bash])
...
async def test_tool_choice_force():
    ...
    session = Session("glm-tool-test-5")
    bash = BashNode(id="bash", session=session, timeout=30.0)
    tools, executor = tools_from_nodes([bash])
...
async def test_parallel_tool_calls_false():
    ...
    session = Session("glm-tool-test-6")
    bash = BashNode(id="bash", session=session, timeout=30.0)
    tools, executor = tools_from_nodes([bash])
...
async def test_tool_choice_auto():
    ...
    session = Session("glm-tool-test-7")
    bash = BashNode(id="bash", session=session, timeout=30.0)
    tools, executor = tools_from_nodes([bash])
...
async def main():
    print("=" * 60)
    print("GLM Tool Calling Feature Tests")
    print("=" * 60)
    ...
    if errors:
        ...
        print("\n✅ All GLM tool calling tests PASSED")
