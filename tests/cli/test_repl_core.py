...
class TestRunInteractive:
    ...
    async def test_run_interactive_creates_local_session(self):
        ...
        with (
            ...
            patch("nerve.core.session.Session") as mock_session_class,
        ):
            ...
            mock_session_class.assert_called_once_with(name="default", server_name="repl")
    ...
    async def test_run_interactive_connects_to_server(self):
        ...
        with (
            ...
            patch("nerve.transport.UnixSocketClient") as mock_client_class,
        ):
            mock_client = Mock()
            ...
            mock_client_class.assert_called_once_with("/tmp/test.sock")
    ...
    async def test_run_interactive_server_disconnect_message(self, capsys):
        ...
        with (
            patch("builtins.input", side_effect=["graphs"]),
            patch(
                "nerve.frontends.cli.utils.get_server_transport",
                return_value=("unix", "/tmp/test.sock"),
            ),
